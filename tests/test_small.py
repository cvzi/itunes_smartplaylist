...
def test_kind_contains_output_uses_original_string(verbose=False):
    ...
    assert "(lower(Uri) LIKE '%.m4a')" in result.query
    assert "(lower(Uri) LIKE '%.m4p')" in result.query
    assert "(lower(Artist) LIKE '%nokia%')" in result.query


def test_kind_contains_broad_text_falls_back_to_text_query(caplog, verbose=False):
    info = (
        "AQEAAwAAAAIAAAAZAAAAAAAAAAcAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAA=="
    ...
    decoded = bytearray(base64.standard_b64decode(criteria))
    decoded = decoded.replace("a\x00a\x00c\x00".encode("latin1"), "a\x00u\x00d\x00i\x00o\x00".encode("latin1"), 1)

    with caplog.at_level(logging.WARNING):
        result = itunessmart.BytesParser(base64.standard_b64decode(info), bytes(decoded)).result

    assert 'Kind contains "audio" ' in result.output
    assert "(lower(Kind) LIKE '%audio%')" in result.query
    assert "(lower(Uri)" not in result.query
    assert "Treating Kind rule as free text instead of structured file-kind match" in caplog.text


def test_kind_is_unknown_value_falls_back_to_text_query(caplog, verbose=False):
    info = (
        "AQEAAwAAAAIAAAAZAAAAAAAAAAcAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAA=="
    ...
    criteria = (
        ...
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAARAAAAAAAAAAgAAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAg"
        "AAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEBAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARJTTHN0AAEAAQAAAAIAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAABAEAAAIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAABAAUAByAGkAbgBjAGUAcwBzAAAACQEAAAEAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAoAdwBlAGkAcgBk"
    )

    with caplog.at_level(logging.WARNING):
        result = itunessmart.Parser(info, criteria).result

    assert 'Artist contains "Princess" ' in result.output
    assert 'Kind is "weird" ' in result.output
    assert result.query == "(lower(Artist) LIKE '%princess%') AND (lower(Kind) = 'weird')"
    assert "Treating Kind rule as free text instead of structured file-kind match" in caplog.text


def test_kind_negative_specific_match_uses_structured_query(verbose=False):
    info = (
        "AQEAAwAAAAIAAAAZAAAAAAAAAAcAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAA=="
    ...
    criteria = (
        ...
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAARAAAAAAAAAAgAAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAg"
        "AAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEBAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAS5TTHN0AAEAAQAAAAIAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAABAEAAAIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAC4AUABlAG8AcABsAGUAIABVAG4AZABlAHIAIABUAGgAZQAgAFMAdABh"
        "AGkAcgBzAAAACQMAAAIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAgAbQBwAGUAZw=="
    )
    result = itunessmart.Parser(info, criteria).result

    assert 'Artist contains "People Under The Stairs" ' in result.output
    assert 'Kind does not contain "mpeg" ' in result.output
    assert "(lower(Uri) NOT LIKE '%.mp3%')" in result.query
    assert "(lower(Uri) NOT LIKE '%.mp4%')" in result.query
    assert "(lower(Kind)" not in result.query


def test_kind_is_known_library_value_uses_structured_query(verbose=False):
    info = (
        "AQEAAwAAAAIAAAAZAAAAAAAAAAcAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAA=="
    ...
    criteria = (
        ...
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAARAAAAAAAAAAgAAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAg"
        "AAAAAAAAAAAAAAAAAAAAAQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEBAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAUBTTHN0AAEAAQAAAAIAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAABAEAAAIAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAABgAQgByAG8AawBlAG4AIABCAGUAbABsAHMAAAAJAQAAAQAAAAAAAAAA"
        "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAMABQAHUAcgBjAGgAYQBz"
        "AGUAZAAgAEEAQQBDACAAYQB1AGQAaQBvACAAZgBpAGwAZQ=="
    )

    result = itunessmart.Parser(info, criteria).result

    assert 'Artist contains "Broken Bells" ' in result.output
    assert 'Kind is "Purchased AAC audio file" ' in result.output
    assert result.query == "(lower(Artist) LIKE '%broken bells%') AND (lower(Uri) LIKE '%.m4a')"
